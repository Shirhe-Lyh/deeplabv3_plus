...
#        residual = self._separable_conv_block(x)
        residual = x
        self._end_points = {}
        for i, layer in enumerate(self._separable_conv_block, 1):
...
class StackBlocksDense(torch.nn.Module):
    ...
    def end_points(self):
        ...
        for layer in self._blocks:
            self._end_points.update(layer.end_points)
        return self._end_points
...
class Xception(torch.nn.Module):
    ...
    def end_points(self):
        ...
        for layer in self._layers:
            if hasattr(layer, '_end_points'):
                for key, value in layer.end_points().items():
                    key_ = '{}/{}'.format(self._scope, key)
                    end_points[key_] = value
        return end_points
